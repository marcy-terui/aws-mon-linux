...
CLOUDWATCH_OPTS="--region $region --namespace System/Linux --dimensions"
...
if [ $CPU_SY -eq 1 ]; then
    ...
    if [ $VERIFY -eq 0 ]; then
        aws cloudwatch put-metric-data --metric-name "CpuSys" --value "$cpu_sy" --unit "Percent" $CLOUDWATCH_OPTS
    fi
fi
